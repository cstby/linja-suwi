...
test: venv-test build.stamp
	TOCHECK=$$(find fonts/variable -type f 2>/dev/null); if [ -z "$$TOCHECK" ]; then TOCHECK=$$(find fonts/ttf -type f 2>/dev/null); fi ; . venv-test/bin/activate; mkdir -p out/ out/fontbakery; fontbakery check-googlefonts -l WARN --exclude-checkid 'com.google.fonts/check/os2_metrics_match_hhea' --exclude-checkid 'com.google.fonts/check/glyphsets/shape_languages' --exclude-checkid 'com.google.fonts/check/glyph_coverage' --exclude-checkid 'com.google.fonts/check/whitespace_widths' --full-lists --succinct --badges out/badges --html out/fontbakery/fontbakery-report.html --ghmarkdown out/fontbakery/fontbakery-report.md $$TOCHECK  || echo '::warning file=sources/config.yaml,title=Fontbakery failures::The fontbakery QA check reported errors in your font. Please check the generated report.'
